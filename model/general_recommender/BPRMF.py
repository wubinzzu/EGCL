...
class BPRMF(AbstractRecommender):
    def __init__(self, sess, dataset, conf):
        super(BPRMF, self).__init__(dataset, conf)
    ...
    def _create_loss(self):
        with tf.name_scope("loss"):
            ...
            if self.is_pairwise is True:
                ...
                self.loss = learner.pairwise_loss(self.loss_function, result) + self.reg_bprmf * l2_loss(p1, q2, q1)
            else:
                self.loss = learner.pointwise_loss(self.loss_function, self.labels, self.output) + \
                            self.reg_bprmf * l2_loss(p1, q1)
